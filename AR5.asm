...
cmd_resetcfg_help:
  DC.B  "RESETCFG (Reset all prefs)",13
  DC.B  "  RESETCFG",13
  DC.B 0
...
cmd_sysram_help:
  DC.B  "SYSRAM (Show system ram blocks)",13
  DC.B  "  SYSRAM",13
  DC.B 0
...
.na
  ...
  MOVE.W #CursorDown,D0
  RTS
.nb
  CMP.B #"C",D0
  BNE.S .nc
  
  MOVE.W #CursorRight,D0
  RTS
.nc
  CMP.B #"D",D0
  BNE.S .nd
...
CMD_RESETCFG:
  ...
  MOVE.W #$10,memoryControlPrefsValue
  LEA LAB_A483AA,A0
  CLR.L (A0)+
  CLR.L (A0)+
  CLR.L (A0)+
  ...
  SF  BootblockCoderPrefsFlag
  CLR.L BootblockCoderValue

  CLR.B DiskCoderDf0Flag
  CLR.B DiskCoderDf1Flag
  CLR.B DiskCoderDf2Flag
  CLR.B DiskCoderDf3Flag
  CLR.L DiskCoderDf0Value
  CLR.L DiskCoderDf1Value
  CLR.L DiskCoderDf2Value
  CLR.L DiskCoderDf3Value
 
  CLR.W DriveControlPrefsValue
  MOVE.B  #3,VirusCheckerSettingsPrefs
  ST.B keymap
  ...
  LEA PrefsRestoredText(PC),A0
  JSR PrintText(PC)
  JSR PrintReady
  RTS

PrefsRestoredText: DC.B "Default preferences restored.",13,0
  even
...
settingsSavedText: DC.B 13,"Settings saved successfully",13,0
  even
...
memoryBlocksText:
  DC.B "System Memory blocks:",13,0
  even
...
.blockloop:
  ...
  CMP.L #1,D7
  BEQ.S .gmode  ;noack for first block
  TST.L D5  ;gmode
  BNE.S .gmode
...
;
;returns error code in D0, block length in D1
GetBlock:
  MOVEM.L D2-D6/A1-A2/A3/A5,-(A7)
  MOVE.B D0,D6    ;expected block num
...
.blockstart
  JSR WaitSerCharTimeout2
  MOVE.B D0,D3  ;block num

  MOVE.W #1,errcode
  JSR WaitSerCharTimeout2 ;inverse block num
  CMP.L #-1,D0
  BEQ.W .blockfail
  MOVE.B D0,D4
...
.getblock:
  ...
  MOVE.W #4,errcode
  JSR WaitSerCharTimeout2 ;crc byte2
  CMP.L #-1,D0
  BEQ.W .blockfail
...
.getmode
  ...
  MOVEQ #0,D2
  CMP.B #"C",D0  ;1k block - with acks
  BEQ.S startsend

  MOVEQ #1,D2
  CMP.B #"G",D0  ;1k block - no acks
  BEQ.S startsend
...
allsent:
  ...
  MOVE.B #"0",(A0)+   ;fsize
  MOVE.B #" ",(A0)+
  MOVE.B #"0",(A0)+ ; file date
  MOVE.B #" ",(A0)+
  MOVE.B #"0",(A0)+   ;?
  MOVE.B #" ",(A0)+
  MOVE.B #"0",(A0)+   ;?
  MOVE.B #" ",(A0)+
  MOVE.B #"1",(A0)+   ;files remaining
  MOVE.B #" ",(A0)+
  MOVE.B #"0",(A0)+   ;sent bytes
.clear
  CLR.B (A0)+
  CMP.L A0,A1
  BNE.S .clear
...
SerPrint:
  ...
  beq.s     .1                    Exit if is the terminating zero.
  cmp.b     #10,D0                Is it a newline?
  bne.s     .notlf
  moveq     #13,D0                If so, output a CR first.
  bsr.w     RawPutChar
  moveq     #$0A,D0
.notlf
  bsr.w     RawPutChar            Output the character.
  bra.s     SerPrint              Go on to next character.
.1
  rts       
...
.trysend  
  ...
  BEQ.S .timeout
  BTST #4,$BFD000
  bne.s     .trysend      ;wait for RTS
  
  move.w    $DFF018,D1             ;Read SERDATR.
  btst      #13,D1                ;Transmitter ready?
  beq.s     .trysend                 ;Wait until true.
...
HelpText:
  ...
  DC.B  "  resetcfg: Reset all preferences                - resetcfg",$D
